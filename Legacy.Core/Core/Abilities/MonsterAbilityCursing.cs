...
{
	public class MonsterAbilityCursing : MonsterAbilityBase
	{
		public MonsterAbilityCursing() : base(EMonsterAbilityType.CURSING_STRIKES)
		{
			...
			if (Random.Range(1f, 100f) > num)
			{
				...
				for (Int32 i = 0; i < p_attackList.Count; i++)
				{
					if (p_attackList[i].Result == EResultType.HIT && !p_isMagic)
					{
						...
						{
							p_character.ConditionHandler.AddCondition(ECondition.CURSED);
							if (p_character.ConditionHandler.HasCondition(ECondition.CURSED))
							{
