		...
		public override void UpdateTurn()
		{
			...
			if (State == EState.IDLE)
			{
				if (m_monsters.Count > 0)
				{
					...
					InitMonsterTurns();
					m_oneAggro = false;
					foreach (Monster monster2 in m_monsters)
					{
						if (monster2.IsAggro)
						{
							m_oneAggro = true;
							if (!flag)
							{
								...
								if (!flag2)
								{
									BaseObjectEventArgs p_eventArgs = new BaseObjectEventArgs(monster2, monster2.Position);
									LegacyLogic.Instance.EventManager.InvokeEvent(this, EEventType.MONSTERS_TURNED_AGGRO, p_eventArgs);
								}
							}
							monster2.AiHandler.DoTurn(grid, party);
						}
						else
						{
							monster2.SkipMovement.Trigger();
						}
					}
