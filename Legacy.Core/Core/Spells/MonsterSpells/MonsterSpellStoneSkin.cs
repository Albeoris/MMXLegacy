...
{
	public class MonsterSpellStoneSkin : MonsterSpell
	{
		public MonsterSpellStoneSkin(String p_effectAnimationClip, Int32 p_castProbability) : base(EMonsterSpell.STONE_SKIN, p_effectAnimationClip, p_castProbability)
		{
		}
		...
		public override void DoEffect(Monster p_monster, Character p_target, SpellEventArgs p_spellArgs)
		{
			...
			for (Int32 i = 0; i < entities.Count; i++)
			{
				if (entities[i] is Monster)
				{
					...
					monster.AddBuff(BuffFactory.CreateMonsterBuff(EMonsterBuffType.STONESKIN, p_monster.MagicPower, m_level));
				}
			}
			m_cooldown = 3;
		}
		...
		public override String GetDescriptionForCaster(MonsterStaticData p_monster)
		{
			...
			return Localization.Instance.GetText("MONSTER_SPELL_STONE_SKIN_INFO", m_descriptionValues);
		}
		...
		public override Boolean CheckSpellPreconditions(Monster p_monster)
		{
			...
			for (Int32 i = 0; i < entities.Count; i++)
			{
				...
				{
					Monster monster = (Monster)entities[i];
					if (!monster.BuffHandler.HasBuff(EMonsterBuffType.STONESKIN))
					{
						return true;
					}
				}
