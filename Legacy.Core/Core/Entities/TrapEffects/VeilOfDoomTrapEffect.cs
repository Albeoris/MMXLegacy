...
{
	public class VeilOfDoomTrapEffect : DamageDealingTrapEffect
	{
		public VeilOfDoomTrapEffect(Int32 p_staticID, InteractiveObject p_parent) : base(p_staticID, p_parent)
		{
		}
		...
		public override void ResolveEffect(Party p_party)
		{
			...
			for (Int32 i = 0; i < 4; i++)
			{
				...
				{
					if (!member.ConditionHandler.HasCondition(ECondition.CURSED))
					{
						conditionsReceived[i] = ECondition.CURSED;
					}
					member.ConditionHandler.AddCondition(ECondition.CURSED);
					conditionsReceived[i] = ECondition.CURSED;
				}
