...
{
	public class GenialAIHandler : MonsterAIHandler
	{
		public GenialAIHandler(Monster p_owner) : base(p_owner)
		{
		...
		public override void FilterPossibleTargets(List<Character> p_characters)
		{
			...
			{
				if (p_characters[i].ConditionHandler.HasOneCondition(ECondition.UNCONSCIOUS | ECondition.PARALYZED | ECondition.STUNNED | ECondition.SLEEPING | ECondition.CONFUSED | ECondition.CURSED))
				{
		...
		internal override Boolean CalculatePath(GridSlot p_start, GridSlot p_target, List<GridSlot> p_pathBuffer)
		{
			...
			{
				AStarHelper<GridSlot>.Calculate(p_start, p_target, 10, m_owner, false, true, true, p_pathBuffer);
			...
			AStarHelper<GridSlot>.Calculate(p_start, list[0], 10, m_owner, false, true, true, p_pathBuffer);
			list.Sort(new Comparison<GridSlot>(DistSortAsc));
			return p_pathBuffer.Count > 0;
		}
	}
