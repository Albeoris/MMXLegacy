...
namespace Legacy.Game.MMGUI
{
	...
	[AddComponentMenu("MM Legacy/MMGUI/UICameraCustom")]
	[ExecuteInEditMode]
	public class UICameraCustom : MonoBehaviour
	{
		...
		public static UICameraCustom current = null;
		...
		private static Int32 CompareFunc(RaycastHit r1, RaycastHit r2)
		{
			return r1.distance.CompareTo(r2.distance);
		}

		private Boolean Raycast(Vector3 inPos, out RaycastHit hit)
		{
			...
			Int32 layerMask = currentCamera.cullingMask & eventReceiverMask;
			Single num = (rangeDistance <= 0f) ? (currentCamera.farClipPlane - currentCamera.nearClipPlane) : rangeDistance;
			Debug.DrawRay(ray.origin, ray.direction * num, Color.red);
			if (clipRaycasts)
			{
				RaycastHit[] array = Physics.RaycastAll(ray, num, layerMask);
				if (array.Length > 1)
				{
					Array.Sort<RaycastHit>(array, new Comparison<RaycastHit>(CompareFunc));
					Int32 i = 0;
					Int32 num2 = array.Length;
					while (i < num2)
					{
						if (IsVisible(ref array[i]))
						{
							hit = array[i];
							return true;
						}
			...
			return Physics.Raycast(ray, out hit, num, layerMask);
		}
		...
		public void ProcessEvents(Vector2 mousePosition)
		{
			current = this;
			currentMousePosition = mousePosition;
			if (useMouse && Application.isPlaying)
			{
				hoveredObject = ((!Raycast(mousePosition, out lastHit)) ? fallThrough : lastHit.collider.gameObject);
				if (hoveredObject == null)
				{
					hoveredObject = genericEventHandler;
				}
				for (Int32 i = 0; i < mMouse.Length; i++)
				{
			...
			current = null;
			currentMousePosition = default(Vector3);
		}
		...
		{
			if (handleEvents && useMouse && Application.isPlaying)
			{
		...
		private void Update()
		{
			if (!handleEvents || !Application.isPlaying)
			{
				return;
			}
			current = this;
			...
			if (useMouse || (useTouch && mIsEditor))
			{
				ProcessMouse(currentMousePosition);
			}
			if (useTouch)
			{
				ProcessTouches();
		...
		public void ProcessMouse(Vector2 mousePosition)
		{
			...
			for (Int32 j = 1; j < mMouse.Length; j++)
			{
			...
			if (useMouse)
			{
				for (Int32 l = 0; l < mMouse.Length; l++)
				{
