...
namespace Legacy.Game.MMGUI
{
	...
	public class DialogView : MonoBehaviour
	{
		...
		[SerializeField]
		private UILabel m_npcName;

		[SerializeField]
		private MultiTextControl m_text;

		[SerializeField]
		private UITexture m_textBackground;

		[SerializeField]
		private UISprite m_npcIcon;

		[SerializeField]
		private GameObject m_entryList;

		[SerializeField]
		private GUIMultiSpriteButton m_closeButton;

		[SerializeField]
		private DialogEntryView[] m_views = Arrays<DialogEntryView>.Empty;

		[SerializeField]
		private GameObject m_goldRessourceViewGO;

		[SerializeField]
		private GameObject m_foodRessourceViewGO;

		[SerializeField]
		private UILabel m_goldRessourceView;

		[SerializeField]
		private UILabel m_foodRessourceView;

		[LocalResourcePath]
		[SerializeField]
		private String m_ConversationBg43;

		[SerializeField]
		[LocalResourcePath]
		private String m_ConversationBg169;

		[SerializeField]
		[LocalResourcePath]
		private String m_ConversationBg1610;
		...
		private void InitTravelDialog(Dialog p_dialog)
		{
			...
			Int32 num2 = CalculatePriceForDifficulty(num, ConfigManager.Instance.Game.NpcTravelFactor);
			String text = LocaManager.GetText(p_dialog.Feature.m_optionKey);
			if (party.Gold < num2)
			{
				p_enabled = false;
			}
			TravelFunction p_function = new TravelFunction(num2, p_dialog.Feature.m_mapName, p_dialog.Feature.m_targetSpawnId);
		...
		private void InitRepairDialog(Dialog p_dialog)
		{
			...
			Int32 num2 = CalculatePriceForDifficulty(num, ConfigManager.Instance.Game.NpcItemRepairFactor);
			Int32 num3 = num2 * brokenItemCount;
			DialogEntryView dialogEntryView = m_views[0];
			String text = LocaManager.GetText("CONVERSATION_REPAIRALL_ENTRY_TEXT", num3);
			...
			if (repairType != ERepairType.WEAPONS)
			{
				...
				{
					text = LocaManager.GetText("CONVERSATION_REPAIRARMOR_ENTRY_TEXT", num3);
				}
			}
			else
			{
				text = LocaManager.GetText("CONVERSATION_REPAIRWEAPONS_ENTRY_TEXT", num3);
			}
			Boolean flag = party.HirelingHandler.Hirelings[0] == LegacyLogic.Instance.ConversationManager.CurrentNpc || party.HirelingHandler.Hirelings[1] == LegacyLogic.Instance.ConversationManager.CurrentNpc;
			if (flag)
			{
			...
			RepairFunction p_function = new RepairFunction(p_dialog.Feature.m_repairType, (!flag) ? num2 : LegacyLogic.Instance.ConversationManager.CurrentNpc.GetCosts(ETargetCondition.REPAIR), LegacyLogic.Instance.ConversationManager.CurrentConversation.RootDialog.ID);
			dialogEntryView.SetFeature(text, p_enabled, p_function);
			m_needGold = true;
		}
		...
		private void InitRestDialog(Dialog p_dialog)
		{
			...
			Int32 num2 = CalculatePriceForDifficulty(num, ConfigManager.Instance.Game.NpcRestFactor);
		...
		private void InitTrainingDialog(Dialog p_dialog)
		{
			...
			for (Int32 i = 0; i < party.Members.Length; i++)
			{
				...
				if (memberByOrder != null)
				{
					...
					Int32 num2 = CalculatePriceForDifficulty(num, ConfigManager.Instance.Game.NpcSkillTrainingFactor);
					String text = LocaManager.GetText("CONVERSATION_TRAINING_ENTRY_TEXT", memberByOrder.Name, num2);
					Boolean flag = true;
					if (memberByOrder.ConditionHandler.HasOneCondition(ECondition.DEAD | ECondition.UNCONSCIOUS | ECondition.PARALYZED | ECondition.SLEEPING))
					{
					...
					{
						text = LocaManager.GetText("CONVERSATION_TRAINING_FAILED_NOT_ENOUGH_GOLD", memberByOrder.Name, num2);
						text = text + " " + LocaManager.GetText("NOT_ENOUGH_GOLD");
						flag = false;
					}
					TrainingFunction p_function = new TrainingFunction(p_dialog.Feature.m_skillID, p_dialog.Feature.m_skillRank, memberByOrder, (p_dialog.Feature.m_dialogID != -1) ? p_dialog.Feature.m_dialogID : p_dialog.ID, num2);
					DialogEntryView dialogEntryView = m_views[i];
					dialogEntryView.SetFeature(text, flag, p_function);
				}
				m_needGold = true;
